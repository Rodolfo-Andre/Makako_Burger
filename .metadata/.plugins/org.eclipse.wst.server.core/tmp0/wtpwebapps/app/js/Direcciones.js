...
const handleOnClick = () => {
	...
	$d.addEventListener("click", (e) => {
		...
		if ($btnAdd == e.target) {
			let contentModal = {
				...
				body: `<form  class="d-flex flex-column gap-4" id="form-add" action="ZonaDeRepartoController" method="POST" style="margin-bottom:30px">
						...
						<div>
							...
							<div id="name-invalid" class="text-start invalid-feedback">Debes seleccionar una dirección del mapa.</div>
						</div>
						
						<input class="" type="hidden" name="txtLongitud" id="Longitud" value=""/>												
					</form>	
					<div id="map" style="height:400px;" class="img-fluid"></div>`,
				footer: `<input form="form-add" type="submit" class="flex-grow-1 btn btn-primary" value="AÑADIR"/>
			...
			}).setView([-11.967309, -77.096901], zoom);
			...
			map.on("dblclick", (ev) => {
				...
				let { lat, lng } = map.mouseEventToLatLng(ev.originalEvent);
				
				if (Object.keys(coordenada).length) coordenada.remove(); 
	
				coordenada = L.marker([lat, lng]).addTo(map).bindPopup(`La coordenada marcada es:<br>${lng}`);
			
				let $txtLatitud = $d.getElementById('Latitud')
				$txtLatitud.value = lat;
				
				let $txtLongitud = $d.getElementById('Longitud')
				$txtLongitud.value = lng
	
				console.log("La coordena es la siguiente:" + lat, lng);
			});
		}
					...
					success: async (json) => {
						...
						if (!encontroZonaReparto) {
							contentModal = {
								...
								footer: `<input form="form-delete" type="submit" class="flex-grow-1 text-white btn btn-danger" value="ELIMINAR"/>
										<button data-bs-dismiss="modal" aria-label="Close" class="flex-grow-1 btn btn-primary">CANCELAR</button>`
							}
						} else {
							contentModal = {
		...
		if ($btnConfirmAdd == e.target) {	
			...
			if (!$inputNameZona.value.match('^([A-Za-zñÑÁÉÍÓÚáéíóú]|.){5,100}[A-Za-zñÑÁÉÍÓÚáéíóú\\d]$')) {
