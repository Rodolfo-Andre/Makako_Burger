	...
	<main class="main">
		<div class="box">
			<div class="header-info">
				...
				<form class="form-search" method="POST" action="CajaController">
			...
			<div class="navigation-info">
				...
				<c:if test="${ totalDePaginacion > 0 }">
					<div>
						<ul class="pagination">
							<c:if test="${  pag > 1 }">
								<a class="link-active" href="CajaController?p=${ pag - 1 }">Anterior</a>
							</c:if>

							<c:forEach var="i" begin="${ comienza }" end="${ termina }">
								<a ${ pag == i ? "class='link-desactive'" : "class='link-active' href='CajaController?p=" += i += "'" }>${ i }</a>
							</c:forEach>

							<c:if test="${ pag < totalDePaginacion }">
								<a class="link-active" href="CajaController?p=${ pag + 1 }">Siguiente</a>
							</c:if>
						</ul>
					</div>
				</c:if>
